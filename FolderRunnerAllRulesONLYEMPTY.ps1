...
#    - gebruikt batching
#    - skipt items die al getagd zijn
# ---------------------------------------------------------

Write-Host "Ophalen van items uit lijst '$listName'..." -ForegroundColor Cyan

# Batch aanmaken waarin alle updates verzameld worden
$batch = New-PnPBatch

# Velden die we nodig hebben uit de lijst
$fields = @(
    "FileRef",
    "FileLeafRef",
    $col_EquipmentDocument,
    $col_NameDocument
)

if ($col_StationLookup) {
    $fields += $col_StationLookup
}
if ($col_MachineCodeLookup) {
    $fields += $col_MachineCodeLookup
}

Get-PnPListItem -List $listName -PageSize 2000 -Fields $fields -ScriptBlock {
    param($items)
    ...
    foreach ($item in $items) {
        ...
        # Optioneel: enkel items onder bepaalde rootfolder
        if ($rootFolderFilter -and ($fileRef -notlike "$rootFolderFilter*")) {
            continue
        }

        # ---------------------------------------------
        # 1. Skip als het item al getagd is
        #    (minstens één van de metadata-kolommen heeft een waarde)
        # ---------------------------------------------
        $alreadyTagged = $false

        if ($col_EquipmentDocument -and $item[$col_EquipmentDocument]) {
            $alreadyTagged = $true
        }
        elseif ($col_NameDocument -and $item[$col_NameDocument]) {
            $alreadyTagged = $true
        }
        elseif ($col_StationLookup -and $item[$col_StationLookup]) {
            $alreadyTagged = $true
        }
        elseif ($col_MachineCodeLookup -and $item[$col_MachineCodeLookup]) {
            $alreadyTagged = $true
        }

        if ($alreadyTagged) {
            Write-Host "Skip item $($item.Id): $fileName (al getagd)" -ForegroundColor DarkGray
            continue
        }

        # ---------------------------------------------
        # 2. Metadata bepalen via je regels
        # ---------------------------------------------
        $extension   = [System.IO.Path]::GetExtension($fileName).ToLower()
        $valuesToSet = Get-MetadataForFile -fileRef $fileRef -fileName $fileName -extension $extension

        # Niets te updaten → volgende
        ...
        # Item toevoegen aan batch (geen directe call naar SharePoint)
...
Write-Host "Klaar. Alle metadata-regels + station- en machine-lookups toegepast (skip voor reeds getagde items)." -ForegroundColor Green
